...
---
<(META)>:
	docid: '73fad44e-6109-4437-a5c2-947f6d1058a4'
	name:
	description: >
		How to connect to the operation of running an account
		trigger and control from an operation file
...
# System
import os, datetime as dt, zipfile, json, errno, logging, requests#		||
import zipfile, logging, datetime
# Third Party
import requests, pytz
#===============================================================================||
from condor import condor, thing
from worldbridge import utils
#===============================================================================||
...
class portal:
	def __init__(self, lwdir=None):
		'set example username and round'
		if lwdir == None:
			lwdir = '/home/solubrew/DataWorkRepo/'
		self.lwdir = lwdir
		pxcfg = f'{here}z-data_/numerai.yaml'
		self.config = condor.instruct(pxcfg).load()
		self.pubAPI()
	...
	def prvAPI(self):
		...
		puid = self.config.session['entanglements']['numerai']['puid']
		skey = self.config.session['entanglements']['numerai']['skey']
		self.napi = api.NumerAPI(puid, skey)
		return self

	def getData(self, fname=None):
		'download current dataset'
		if fname == None:
			fname = thing.when().dtid
		self.napi.download_current_dataset(self.lwdir, f'{fname}.zip',
																unzip=False)#	||
		return self

	def getAllcomps(self):
		all_competitions = self.napi.get_competitions()# get competitions

	def getLeaderboard(self, rnd=None):
		'get leaderboard for the current round'
		if rnd == None:
			self.leaderboard = napi.get_leaderboard()
		else:
			self.leaderboard = napi.get_leaderboard(round_num=rnd)
		return self

	def submit(self, cfg):
		'''upload predictions'''
		self.prvAPI()
		for mid, path in cfg.items():
			submissions[mid] = self.napi.upload_predictions(path, model_id=mid)
#		self.checkSubmit()
		return submissions
...
class NumerAPI(object):
	...
	def get_dataset_url(self, tournament=8):
		...
		"""
		query = """query($tournament: Int!) {dataset(tournament: $tournament)}"""
	...
	def download_current_dataset(self, dest_path=".", dest_filename=None,
		...
		if dest_filename is None:
			...
			if unzip and not dest_filename.endswith(".zip"):#					||ensure it ends with ".zip"
				dest_filename += ".zip"
		dataset_path = os.path.join(dest_path, dest_filename)
		if os.path.exists(dataset_path):
			self.logger.info("target file already exists")
			return dataset_path
		utils.ensure_directory_exists(dest_path)#								||create parent folder if necessary
		url = self.get_dataset_url(tournament)
		utils.download_file(url, dataset_path, self.show_progress_bars)#		||unzip dataset
		if unzip:
			fonql.uncompress(dataset_path, dest_path, dest_filename[:-4])
			#utils.unzip_file(dataset_path, dest_path, dest_filename[:-4])
		return dataset_path
	...
	def raw_query(self, query, variables=None, authorization=False):
		...
		result = utils.post_with_err_handling(API_TOURNAMENT_URL, body, headers)
		if result and "errors" in result:
			err = self._handle_call_error(result['errors'])
			raise ValueError(err)# fail!
		return result
	...
	def get_staking_leaderboard(self, round_num=0, tournament=8):
		...
		for s in stakes:
			utils.replace(s["stake"], "insertedAt", utils.parse_datetime_string)
